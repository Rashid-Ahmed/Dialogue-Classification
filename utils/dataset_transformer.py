...
class DiscourseDataloader(Dataset):
  ...
  def __getitem__(self, index):
    ...
    if self.target_type != 'soc':
      targets = batch['epi']
      targets = get_one_hot(offset, targets)
      targets = get_splitted_data(self.target_type, targets)
    else:
      sentences, targets = clean_soc(batch)
      sentences = sentences.tolist()
    #Converting sentences into embedding vectors
    
    embeddings = self.tokenizer(sentences, padding=True, truncation=True, max_length = 55, return_tensors='pt')
